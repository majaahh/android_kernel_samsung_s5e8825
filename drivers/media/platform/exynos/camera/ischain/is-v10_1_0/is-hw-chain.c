	...
	case DEV_HW_MCSC0:
		{
			...
			cap->hw_ver = HW_SET_VERSION(10, 1, 0, 0);
			cap->max_output = 5;
			cap->max_djag = 1;
			cap->max_cac = 1;
			cap->max_uvsp = 2;
...
int is_hw_get_capture_slot(struct is_frame *frame, u32 **taddr, u64 **taddr_k, u32 vid)
{
	...
	case IS_LVN_LME_PROCESSED:
		...
		break;
	case IS_LVN_LME_MBMV0:
		*taddr = frame->lmembmv0TargetAddress;
		break;
	case IS_LVN_LME_MBMV1:
		*taddr = frame->lmembmv1TargetAddress;
		break;
	case IS_LVN_LME_SAD:
		*taddr = frame->lmesadTargetAddress;
		if (taddr_k)
			*taddr_k = frame->lmesadKTargetAddress;
		break;
	case IS_LVN_LME_PROCESSED_HDR:
		/* No DMA out */
		if (taddr_k)
			*taddr_k = frame->lmemhdrKTargetAddress;
		break;
	/* MCFP */
	...
	bool reset)
{
	...
	case DEV_HW_LME0:
		...
		TADDR_COPY(dst, src, lmembmv0TargetAddress, reset);
		TADDR_COPY(dst, src, lmembmv1TargetAddress, reset);
		TADDR_COPY(dst, src, lmesadTargetAddress, reset);
		TADDR_COPY(dst, src, lmesadKTargetAddress, reset);
		TADDR_COPY(dst, src, lmemKTargetAddress, reset);
		TADDR_COPY(dst, src, lmemhdrKTargetAddress, reset);
		break;
	case DEV_HW_BYRP:
