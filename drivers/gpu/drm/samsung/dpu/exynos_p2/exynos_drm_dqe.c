...
static char *dqe_acquire_colormode_ctx(struct exynos_dqe *dqe)
{
	...
	gl_hdr = (struct dqe_colormode_global_header *)dqe->colormode.dma_vbuf;
				...
				struct drm_crtc_state *crtc_state)
{
	...
	const char *data;
	u32 *lut;
	u16 size, remain, len, count = 0, count_cgc = 0;
	struct exynos_drm_crtc_state *exynos_crtc_state =
					to_exynos_crtc_state(crtc_state);
				...
				struct dqe_ctx_int *lut, const char *buffer, size_t count)
{
	...
	ret = dqe_conv_str2lut(buffer, count, lut->gamma_matrix, ARRAY_SIZE(lut->gamma_matrix), false);
	if (ret < 0)
		return ret;
	if (mode_idx == DQE_MODE_MAIN) // do not update dqe_ctx_reg for preset LUTs
