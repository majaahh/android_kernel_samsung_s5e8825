...
{
	if (n_6ghz_params)
		return sizeof(struct slsi_mlme_parameters) +
		       (n_6ghz_params * sizeof(struct slsi_mlme_scan_6ghz_descriptor));
	else
		return 0;
}
...
{
	struct slsi_mlme_parameters mlme_6ghz_header = {0};
	struct slsi_mlme_scan_6ghz_descriptor *mlme_6ghz_desc = 0;
	u32 mlme_6ghz_desc_size = n_6ghz_params * sizeof(struct slsi_mlme_scan_6ghz_descriptor);
	u8 *scan_6ghz_param_header = 0;
	int i = 0;
	...
	mlme_6ghz_header.element_id = SLSI_WLAN_EID_VENDOR_SPECIFIC;
	mlme_6ghz_header.length = mlme_6ghz_desc_size + SLSI_MLME_PARAM_HEADER_SIZE;
	slsi_mlme_put_oui(mlme_6ghz_header.oui, SLSI_MLME_SAMSUNG_OUI);
	slsi_mlme_put_oui_type(&mlme_6ghz_header, SLSI_MLME_TYPE_SCAN_PARAM, SLSI_MLME_SUBTYPE_6GHZ_PARAM);

	scan_6ghz_param_header = fapi_append_data(req, (u8 *)&mlme_6ghz_header, sizeof(struct slsi_mlme_parameters));
	if (!scan_6ghz_param_header) {
		SLSI_WARN(sdev, "scan_6ghz_param_header append failed\n");
		kfree_skb(req);
		return -EINVAL;
	}

	mlme_6ghz_desc = (struct slsi_mlme_scan_6ghz_descriptor *)fapi_append_data(req, NULL, mlme_6ghz_desc_size);
	if (!mlme_6ghz_desc) {
		SLSI_WARN(sdev, "mlme_6ghz_descriptor append failed\n");
		kfree_skb(req);
		return -EINVAL;
	}

	for (i = 0; i < n_6ghz_params; i++) {
		if (scan_6ghz_params[i].short_ssid_valid)
			mlme_6ghz_desc[i].short_ssid = cpu_to_le32(scan_6ghz_params[i].short_ssid);
		mlme_6ghz_desc[i].channel_idx = cpu_to_le32(scan_6ghz_params[i].channel_idx);
		memcpy(mlme_6ghz_desc[i].bssid, scan_6ghz_params[i].bssid, ETH_ALEN);

		if (scan_6ghz_params[i].unsolicited_probe)
			mlme_6ghz_desc[i].scan_6ghz_option |= SLSI_UNSOLICITED_PROBE;
		if (scan_6ghz_params[i].short_ssid_valid)
			mlme_6ghz_desc[i].scan_6ghz_option |= SLSI_SHORT_SSID_VALID;
		if (scan_6ghz_params[i].psc_no_listen)
			mlme_6ghz_desc[i].scan_6ghz_option |= SLSI_PSC_NO_LISTEN;
	}
	return 0;
}
#endif
					 ...
					 bool                               passive_scan)
{
	...
	if (scan_type == FAPI_SCANTYPE_P2P_SCAN_FULL) {
		...
		p[3] = 0; /* ToDo: 6GHz policy need to be updated */
		channels_list_ie_head->length += SLSI_SCAN_CHANNEL_DESCRIPTOR_SIZE;
		return 0;
	}
	...
	for (i = 0; i < num_channels; i++) {
		...
		freq_fw_unit = 2 * ieee80211_channel_to_frequency(chann, channels[i]->band);
			     ...
			     u16                                ies_len)
{
	...
	if (request->n_scan_plans > 1 && request->scan_plans[0].interval < request->scan_plans[1].interval) {
		...
		scan_timing_ie.exponent = 1;
		scan_timing_ie.step_count = 0;
	}
#else
...
int slsi_mlme_start(struct slsi_dev *sdev, struct net_device *dev, u8 *bssid, struct cfg80211_ap_settings *settings, const u8 *wpa_ie_pos, const u8 *wmm_ie_pos, bool append_vht_ies)
{
	...
	if (cfg80211_find_ext_ie(WLAN_EID_EXT_HE_CAPABILITY, settings->beacon.tail, settings->beacon.tail_len)) {
